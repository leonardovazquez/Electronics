...
Public Sub Initialize
	...
	If admin.IsEnabled = False Then
		If admin.Enable = False Then
			ToastMessageShow("Error habilitando el bluetooth", True)
		Else
			ToastMessageShow("Habilitando el módulo bluetooth...", False)
		End If
	Else
		BluetoothState = True
	End If
...
Private Sub AStream_NewData (Buffer() As Byte)
	CallSub2(Controltanque, "NewMessage", BytesToString(Buffer, 0, Buffer.Length, "UTF8"))
End Sub
...
Private Sub NotifyOfStateChanged
	For Each Target In Array(Main, Controltanque)
